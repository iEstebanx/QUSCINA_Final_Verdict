...
// MUI Icons (MenuIcon REMOVED here)
...
import iconLogo from "@/assets/iconLogo.png"; // <-- your logo

function NavLeaf({ to, label, icon: Icon, collapsed }) {
  const base = { display: "flex", alignItems: "center", gap: 1.25, width: "100%", px: 1.5, py: 1.1, borderRadius: 1.5, typography: "body2" };
  return (
...
function NavGroup({ label, icon: Icon, collapsed, children, open: controlledOpen, onToggle, active }) {
  const Row = (
    <ButtonBase
      onClick={onToggle}
      sx={{
      ...
      {!collapsed && (
        <Box sx={{ display: "flex", alignItems: "center", gap: 1, flex: 1 }}>
          ...
          <Box sx={{ ml: "auto", display: "flex", alignItems: "center" }}>
            {controlledOpen ? <ExpandLessIcon sx={{ fontSize: 18 }} /> : <ExpandMoreIcon sx={{ fontSize: 18 }} />}
          </Box>
        </Box>
      )}
    </ButtonBase>
...
export default function Sidebar({ collapsed, onToggle, width = 240, collapsedWidth = 72 }) {
  const { user } = useAuth();
  ...
  return (
    <Box
      sx={{
        ...
        transition: (theme) => theme.transitions.create(["width"], { easing: theme.transitions.easing.sharp, duration: theme.transitions.duration.shortest }),
        overflowX: "hidden",
      }}
    >
      {/* Top: brand only (icon always, text when expanded) */}
      ...
      <Box sx={{ px: 1, display: "grid", gap: 0.5 }}>
        ...
        <NavGroup label="Menu" icon={RestaurantMenuIcon} collapsed={collapsed} open={openMenu} onToggle={() => setOpenMenu(v => !v)} active={isMenuActive}>
        ...
        <NavGroup label="Inventory" icon={Inventory2Icon} collapsed={collapsed} open={openInv} onToggle={() => setOpenInv(v => !v)} active={isInventoryActive}>
        ...
        <NavGroup label="Settings" icon={SettingsIcon} collapsed={collapsed} open={openSettings} onToggle={() => setOpenSettings(v => !v)} active={isSettingsActive}>
