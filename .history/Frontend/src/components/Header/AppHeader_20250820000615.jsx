...
export default function AppHeader({
  ...
  const labelMap = { "": "Home", dashboard: "Dashboard", users: "Users", reports: "Reports" };
  ...
  const crumbs = useMemo(() => {
    ...
    return segments.map((seg, idx) => {
      ...
      return isLast ? (
        <Typography key={pathAcc} color="text.primary">{label}</Typography>
      ) : (
        <Link key={pathAcc} component={RouterLink} underline="hover" color="inherit" to={pathAcc}>
          {label}
        </Link>
      );
    });
  ...
  return (
    <AppBar
      ...
      sx={{
        ...
        left: leftOffset,
        width: { xs: "100%", sm: `calc(100% - ${collapsed ? collapsedWidth : width}px)` },
    ...
    >
      <Toolbar sx={{ minHeight: APPBAR_HEIGHT, gap: 2 }}>
        ...
        <IconButton edge="start" onClick={onToggle} aria-label="toggle sidebar">
          <MenuIcon />
        </IconButton>

        {/* Optional thin divider after hamburger */}
